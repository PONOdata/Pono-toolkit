# Definitive AMD Ryzen mobile SoC identification. Ignores marketing names,
# uses raw CPUID Family/Model/Stepping. Use this to answer "do I have XDNA 1
# or XDNA 2 on this box" without trusting the CPU's marketing string.
#
# Known signatures (as of 2026-04):
#   Hawk Point (Phoenix 2 refresh, XDNA 1, ~16 TOPS): Family 25 (0x19) Model 117 (0x75)
#   Phoenix (original, XDNA 1, ~10 TOPS):             Family 25 (0x19) Model 116 (0x74)
#   Strix Point (XDNA 2, ~50 TOPS):                   Family 26 (0x1A) Model 36 (0x24)
#   Krackan Point (XDNA 2, ~50 TOPS):                 Family 26 (0x1A) Model 32 (0x20)
#   Strix Halo (XDNA 2, ~50 TOPS):                    Family 26 (0x1A) Model 96 (0x60)
#
# Read-only. No admin.
...
foreach ($c in $cpus) {
  ...
  Write-Host ("  Cores:         {0}" -f $c.NumberOfCores)
  Write-Host ("  LogicalProcs:  {0}" -f $c.NumberOfLogicalProcessors)
  Write-Host ("  MaxClock:      {0} MHz" -f $c.MaxClockSpeed)
  Write-Host ("  ProcessorId:   {0}" -f $c.ProcessorId)
}

Write-Host ""
Write-Host "=== CPUID (from Registry — what Windows sees at boot) ==="
...
Write-Host ""
Write-Host "=== iGPU (Strix Point ships 880M/890M; Hawk Point ships 780M; Phoenix ships 780M) ==="
